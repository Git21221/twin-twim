...
export const fetchAvailableUsers = createAsyncThunk<User[], void, { rejectValue: string }>(
  "users/fetchAvailableUsers",
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAvailableUsers.fulfilled, (state, action: PayloadAction<User[]>) => {
        state.loading = false;
        state.users = action.payload;
      })
      .addCase(fetchAvailableUsers.rejected, (state, action: PayloadAction<string | undefined>) => {
        state.loading = false;
      ...
      .addCase(fetchUserProfile.rejected, (state, action: PayloadAction<string | undefined>) => {
        state.loading = false;
        state.profile = null;
        state.error = action.payload || "An error occurred while fetching the user profile.";
      });
  },
});
